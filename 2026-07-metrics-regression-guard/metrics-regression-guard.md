...
- **Caveat for the design (§8 "bless HEAD-truth"):** here HEAD-truth is a *worse* number than served — blessing it locks in the genesis-at-$0 over-count. **Open, unresolved:** *why* does the current recompute store 0 where served stored NULL for the same cohort? No `ifNull`/`coalesce` in the seam SQL, and the guard `acquisition_price` col is `Nullable(Float64)` — yet 0 NULLs / 41 037 zeros (and the count differs from served's 57 053, so not a clean 1:1 NULL→0 flip). Could be a seam write-path coercion, a guard-table DDL default, or a genuine code change since served was computed. **Decides fix-vs-bless.** Next: git archaeology on the seam write path + the acquisition-price ASOF thread (§0: `352af466` toStartOfHour→toStartOfFiveMinute; PR #2132 INNER-JOIN cohort drop), and/or a clean seam re-run to see whether HEAD now emits NULL (→ matches served) or 0.
