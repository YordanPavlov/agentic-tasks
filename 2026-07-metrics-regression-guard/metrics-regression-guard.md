...
**RESOLVED — the NULL→0 is a side-effect of the ASOF-join fix (git archaeology, 2026-07-08):**
- `84ec2023` "Add acquisition price to distribution deltas" (**2025-03-19**, = PR **#2132**): introduced acquisition_price via **`INNER JOIN`** → genesis cohort (no price) **dropped** = the #2132 data-loss (matches journal §0 window 2025-03-19 → 2026-03-02).
- `055dd3d7` "Fix age distribution job" (**2026-03-03**, day after the bug was noticed): `INNER JOIN` → **`LEFT ASOF JOIN`** → genesis cohort **restored but priced at 0**.
- **Mechanism:** `join_use_nulls` is **not set anywhere** in `daily_metrics/` (grep empty) → ClickHouse default `0` → a LEFT/LEFT-ASOF join fills unmatched rows with the **type default (`0` for Float64), not NULL** (the fill happens in the join, before storage — the `Nullable` column type is irrelevant). So the LEFT-ASOF fix corrected the *data loss* but introduced a *valuation error*: the genesis/premine cohort now books at acquisition_price 0 → 100% of current value counted as profit → NPL 281M → 559M.
