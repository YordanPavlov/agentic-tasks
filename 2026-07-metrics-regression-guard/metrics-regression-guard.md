...
### 14.6 First airflow-dev run failed opaque; gate log-visibility fixed (2026-07-17)

Operator ran the full setup (airflow-dev on AWS stage, image clickhouse-tables:metricsQA). `guard-assert-write-surface-eth` pod died sub-second (started/terminated 13:28:01, exit 1) and the Airflow log shows nothing: the gate command redirected main.py's stdout+stderr into /tmp/dry.sql, which died with the pod. Fixed in docker-airflow `42fa0177`: on non-zero rc the pod echoes `tail -n 200 /tmp/dry.sql` and exits with the original rc (gate stays strict).

Likely root cause (to confirm on retry with visible logs): the `regression_guard` CH user is only on the unmerged devops branch `regressionGuardUser` (d58f1e78) → not deployed on stage CH → instant auth failure. Second suspect: stage CH health (NodePort clickhouse.stage.san:30900 reset every connection from the agent container today, even SELECT 1 as readonly). Pod env was otherwise correct: 54 jobs incl. daily-prices-and-volumes appended, regression_guard user + redirects, window 2015→2017. Note: the failing run was `scheduled__2026-07-09T03:00` attempt 5/6 — a stale scheduled run (current DAG is unscheduled off production); fresh manual trigger recommended after redeploy.

Stage prereqs still outstanding (per §12.8/§14): deploy regression_guard user (merge devops branch or apply usersd), `make guard_tables` against stage, verify stage asset_prices_v3 window coverage.
