...
**Incidental finding:** XRP seam `dt` is second-resolution (ledger-close), NOT 5-min aligned — in prod (90%) and exp (98.8%) alike. Any comparison/query joining the seam to a 5-min price grid MUST `toStartOfFiveMinute(dt)` first (this bug produced a false 48× discrepancy mid-session before being caught).
