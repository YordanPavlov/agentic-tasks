...
**Status: action plan from the 2026-07-13 brainstorming session. No code yet.**
...
6. **Verdict = boolean decision + metadata**, never a bare boolean:
   ...
   location/project feed clustering and digest readability.
...
7. **Architecture — `main.py` (cron) + `digest` (interactive), SQLite truth:**
   ...
   **Two timestamps, one writer each:** `judged_at` set by the run for EVERY
   ...
   Run dir is disposable only after ingest; keep the previous run for
   postmortems, delete run N-1 when starting run N.
...
8. **Seen-store (set-difference model, skip-tolerant by construction):**
   ...
   honor rel=canonical); content identity = hash of *extracted text* (verdict
   cache key; re-judge on change; conditional GETs via ETag/Last-Modified);
   story identity = cluster by entity/embedding, notify per cluster. Search
...
- **M0 — scaffold + spec-by-test:** repo layout (`cli.py db.py sources/
  ...
  (`pages(url_canon PK, first_seen_at, published_at, last_fetched_at, etag,
  content_hash, verdict…, judged_at, cluster_id)`, `cursors`, `clusters` with
  `presented_at`), fixtures committed, unit suites for extract /
  canonicalize / db / digest written first.
